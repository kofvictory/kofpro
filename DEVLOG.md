...
- **フェーズ4**: 音声入力、画像添付、能動的リマインド ← `docs/IDEA_BACKLOG.md` §2(PET化)と対応
- **NPU化（実験）**: npurun へ `OLLAMA_HOST=http://localhost:11435` で接続 ← 同 §3/§5

> **将来構想の一次情報は `docs/IDEA_BACKLOG.md`**（EP01スコープ外・未実装・参考のみ）。
> 上記ロードマップはその要約。着手が決まった項目のみ本ロードマップ/変更履歴へ昇格する。
> バックログとの対応: §1スタイルチェンジ型モード(未) / §2 PET化(=フェーズ4) /
> §3 調査ナビ+NPU自律収集(未・EP02縦糸) / §4 チャット履歴確認ツール(未。🗑クリアは
> 履歴を消す逆方向で永続化なし=別物) / §5 ローカル/NPUモデル候補(既定 qwen2.5・Qwen3が本命候補) /
> §6 賢いモードAPIコスパ定期レビュー(現行基準 claude-haiku-4-5)。
...
- 2026-07-05｜docs｜台本本体をリポジトリに取り込み(docs/ep01/EP01_SCRIPT.md) — 手直し用v2を正として集約。レビューの3差分を適用(⑤Dフォールバックをチップ基準に/⑥図解を純CPUに/②弱点素材を造語クリップに)、⑦の数値を実測確定値に更新(モデル読込20s・初回フル応答150s・生成タイムアウト240s、⑥図のNPU0%/CPU約69%も確定値化)。以後は台本もリポジトリ側を編集し二重管理を解消
- 2026-07-05｜docs｜docs/IDEA_BACKLOG.md をリポジトリに統合(参考のみ・EP01スコープ外・未実装) — §1スタイルチェンジ型モード/§2 PET化(=ロードマップ フェーズ4)/§3 調査ナビ+NPU自律収集(EP02縦糸)/§4 チャット履歴確認ツール(未・🗑クリアとは別物)/§5 ローカル/NPUモデル候補(Qwen3が本命候補)/§6 賢いモードAPIコスパ定期レビュー(基準 claude-haiku-4-5)。DEVLOGロードマップから相互参照を張り整合を確認
- 2026-07-08｜docs｜戦略セッション(別ブランチ)の最終ゲート成果をEP01資料へ反映 — ①台本(EP01_SCRIPT.md): ⓪コールドオープン新設(冒頭15秒で確定タイトル「NPU0%」の約束を回収・追加撮影ゼロ)、④ビートB圧縮(40s→約25s・3点均等をやめ対比表1枚+②の罠だけ深掘り)、①メタ構造小ネタを任意→必須へ昇格(制作タスクをinboxに仕込む協働の可視化)、⑤`mutated:true`おまけカット確定、確定タイトルA/B案+タイムコード方針を明記、微修正(OP③「初めて反応して動く」→「初めて仕事をして見せる」)。②ナレ(EP01_NARRATION_CHECKLIST.md): ⓪コールドオープン2行を7/21収録バッチに追加。③公開メタ(EP01_POST_UPLOAD_MONETIZATION.md 2-2/2-3): チャンネル名「コフと僕」確定・チャンネル概要/概要欄冒頭ブロック/固定コメントの確定文面を転記(本業露出は中立で未言及)。④進捗(EP01_SHOTLIST_PROGRESS.md): 🟡2本(⑦ビフォー誤フォールバック・①⑤別人感)を🔁撮り直し確定(7/13実施)へ、7/13午後の撮影メニュー(撮影順8ステップ・破壊的env var操作は最後)を追加
